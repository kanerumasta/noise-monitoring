...
import { TNode, TNoiseRecord, TNoiseRecordWithNode } from "@/schemas/node_schemas";



export const fetchEvents = async (selectedNode: string): Promise<TNoiseRecordWithNode[]> => {
  ...
  if (selectedNode === "all") {
    ...
    const nodeRef = doc(db, "nodes", selectedNode);
