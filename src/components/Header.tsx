...
const Header = () => {
  ...
  const notifications = useNotifications().filter((d)=>d.isRead === false)
  ...
  return (
    <header className="bg-white shadow-sm h-16 flex items-center justify-between px-6 relative z-50">
      <div className="text-xl font-semibold text-gray-800">Noise Monitoring</div>
      <div className="flex items-center space-x-5">
        ...
        {dropdownOpen && (
  <div
    className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-white border border-gray-200 rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 scrollbar-hide"
    style={{
      scrollbarWidth: 'none',
        ...
        {notifications.map((notif) => {
          ...
          >
            <div className="flex justify-between items-center">
                <span className={`${tier.iconColor} flex-shrink-0`}>{tier.icon}</span>
              ...
              >
                {new Date(notif.timestamp.toDate()).toLocaleDateString()}
              </span>
        }
            </div>
            <p className="mt-2 text-gray-400 line-clamp-2 text-sm">{notif.message}</p>
          </li>)
})}
      </ul>
    )}
    <button onClick={handleToNotifications} className='p-8 text-gray-600 w-full hover:bg-gray-100 hover:font-boldx'>See all notifications</button>
  </div>
)}
        ...
        <div className="relative">
          <EnvelopeIcon className="w-6 h-6 text-gray-600 hover:text-indigo-600 transition cursor-pointer" />
          <span className="absolute -top-1 -right-1 bg-red-600 text-white text-xs font-semibold rounded-full w-5 h-5 flex items-center justify-center shadow-lg">
            4
          </span>
        </div>
